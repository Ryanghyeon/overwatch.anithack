...
export function useOverwatch(battletag) {
  ...
  const [owStats, setOwStats] = useState({
    quickplay: null,
    competitive: null,
  });
    ...
    const fetchOwData = async () => {
      ...
      try {
        ...
        const [summaryRes, heroesRes] = await Promise.all([
          fetch(
            `https://overfast-api.tekrop.fr/players/${formattedTag}/summary`,
          ),
          fetch("https://overfast-api.tekrop.fr/heroes"),
        ]);

        if (!summaryRes.ok)
          throw new Error("계정을 찾을 수 없거나 잘못된 배틀태그입니다.");
        ...
        try {
          ...
          const [qpSumRes, compSumRes] = await Promise.all([
            fetch(
              `https://overfast-api.tekrop.fr/players/${formattedTag}/stats/summary?gamemode=quickplay`,
            ),
            fetch(
              `https://overfast-api.tekrop.fr/players/${formattedTag}/stats/summary?gamemode=competitive`,
            ),
          ]);
