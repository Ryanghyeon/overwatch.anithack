...
export const useAccountManagement = () => {
    ...
    // 1. 프로필 업데이트 로직
    const updateProfile = async (targetUserData, editUsername, battletagInput, finalPhotoUrl) => {
        ...
        try {
            ...
            // 닉네임 중복 체크
            const isUsernameTaken = await checkUsername(editUsername, currentUser.uid);
            if (isUsernameTaken) {
            ...
            // 배틀태그 중복 체크
            if (battletagInput) {
                const isBattletagTaken = await checkBattletag(battletagInput, currentUser.uid);
                if (isBattletagTaken) {
                    setIsProcessing(false);
