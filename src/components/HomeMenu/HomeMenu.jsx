  ...
  if (!user) {
    return (
      <div className="menu-buttons" style={{ marginTop: "30px" }}>
        ...
        <Link to="/ranking" className="btn-action">
          🏆 신고 랭킹
        </Link>
      </div>
    );
  ...
  return (
    <div className="user-menu" style={{ marginTop: "30px" }}>
      ...
      <div className="menu-buttons">
        <Link to="/report" className="btn-action highlight">
          🚨 신고하기
        </Link>
        <Link to="/ranking" className="btn-action">
          🏆 신고 랭킹
        </Link>
        <Link to="/mypage" className="btn-action">
          👤 마이페이지
        </Link>

        {isAdmin && (
          <Link to="/admin" className="btn-action admin-btn">
            🛠 관리자 대시보드
          </Link>
        )}

        <button className="btn-action outline" onClick={handleLogout}>
          로그아웃
        </button>
      </div>
    </div>
