...
const MyProfileSection = ({
  ...
  battletagInput, setBattletagInput,
  onSave, onDelete,
  ...
  <div className="profile-section section-me">
    ...
    <button onClick={onSave} className="btn-save btn-profile-save" disabled={isProcessing}>
    ...
    {isPasswordUser && (
      <div className="security-zone">
        ...
        {!showPasswordChange ? (
          <button onClick={() => setShowPasswordChange(true)} className="btn-outline">
            비밀번호 변경하기
          </button>
        ) : (
          <div className="password-change-box">
            <input type="password" placeholder="현재 비밀번호" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} className="profile-input" style={{ marginBottom: "10px" }} />
            <input type="password" placeholder="새 비밀번호 (6자리 이상)" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="profile-input" style={{ marginBottom: "10px" }} />
            <input type="password" placeholder="새 비밀번호 확인" value={confirmNewPassword} onChange={(e) => setConfirmNewPassword(e.target.value)} className="profile-input" style={{ marginBottom: "15px" }} />

            <div className="delete-btn-group">
              <button onClick={onChangePassword} className="btn-save" disabled={isProcessing} style={{ flex: 1, marginTop: 0, padding: "12px" }}>
                {isProcessing ? "처리 중..." : "변경 완료"}
              </button>
              <button onClick={() => {
                setShowPasswordChange(false); setCurrentPassword(""); setNewPassword(""); setConfirmNewPassword("");
    ...
    <div className="danger-zone">
      {!showDeleteConfirm ? (
        <button onClick={() => setShowDeleteConfirm(true)} className="btn-delete-account">
          회원 탈퇴
        </button>
      ) : (
        <div className="delete-confirm-box">
          {isPasswordUser ? (
            <>
              <p className="delete-warning-text">탈퇴하려면 비밀번호를 입력해 주세요.</p>
              <input
            ...
            <p className="delete-warning-text discord-warning">
              디스코드 연동 계정입니다.<br />아래 버튼을 누르면 즉시 영구 탈퇴 처리됩니다.
            </p>
          )}

          <div className="delete-btn-group">
            <button onClick={() => onDelete(deletePassword)} className="btn-delete-account" disabled={isProcessing}>
              {isProcessing ? "처리 중..." : "영구 탈퇴"}
            </button>
            <button onClick={() => { setShowDeleteConfirm(false); setDeletePassword(""); }} className="btn-outline">
              취소
            </button>
          </div>
        </div>
...
const AdminSection = ({ userData, reportCount, onBan }) => (
  <div className="profile-section section-admin">
    ...
    <p className="admin-info"><strong>UID:</strong> {userData.uid}</p>
    <p className="admin-info"><strong>Email:</strong> {userData.email || "비공개(디스코드 가입 등)"}</p>
    <p className="admin-info"><strong>작성한 신고:</strong> {reportCount}건</p>
    <button onClick={onBan} className="btn-ban">이 유저 강제 정지하기</button>
  </div>
);
...
export function Profile() {
  ...
  const { isProcessing, updateProfile, deleteAccount, changePassword } = useAccountManagement();
  ...
  const handleSave = async () => {
    if (!editUsername.trim()) { alert("유저네임을 입력해 주세요."); return; }
    if (battletagInput.trim() && !isValidBattletag(battletagInput)) { alert("올바른 배틀태그 형식이 아닙니다."); return; }
    ...
    await updateProfile(targetUserData, editUsername, battletagInput.trim(), finalPhotoUrl);
  };
  ...
  const handlePasswordChangeSubmit = async () => {
    ...
    if (newPassword.length < 6) return alert("새 비밀번호는 최소 6자리 이상이어야 합니다.");
  ...
  const handleAdminBan = () => alert(`[관리자 권한] ${targetUserData.username} 유저를 제재했습니다. (기능 구현 예정)`);
      ...
      <div className="profile-box">
        ...
        {isMe && (
          <MyProfileSection
            ...
            battletagInput={battletagInput} setBattletagInput={setBattletagInput}
            onSave={handleSave} onDelete={handleDeleteAccount}

            showDeleteConfirm={showDeleteConfirm} setShowDeleteConfirm={setShowDeleteConfirm}
            deletePassword={deletePassword} setDeletePassword={setDeletePassword}

            // ✨ 새로 추가된 프롭스들 전달
            showPasswordChange={showPasswordChange} setShowPasswordChange={setShowPasswordChange}
        ...
        {!isMe && isAdmin && (
          <AdminSection userData={targetUserData} reportCount={userReportCount} onBan={handleAdminBan} />
        )}
